...
`@sarmay/kaz-converter-lm` 不绑定某一个模型仓库。  
...
`@sarmay/kaz-converter` 设计上就是浏览器友好的：
...
直接使用 `@sarmay/kaz-converter`。

### 需要更高的阿拉伯文 -> 西里尔文精度

安装两个包，并使用 `@sarmay/kaz-converter-lm`。
...
1. 先发 `@sarmay/kaz-converter`
2. 再发 `@sarmay/kaz-converter-lm`
